 ...
 * by the source-import position of the CSS modules — rather than by the
 * non-source-order of `chunk.imports` / `chunk.viteMetadata.importedCss`.
 */
...
export function cssPostPlugin(config: ResolvedConfig): Plugin {
  ...
  return {
    ...
    async renderChunk(code, chunk, opts, meta) {
      ...
      // concatenated in the order the source code imports them. The
      // chunk-record order (`Object.keys(chunk.modules)`) is determined
      // by Rollup's chunking and does not reliably match source order.
      ...
      if (chunkCSS !== undefined) {
        ...
        if (this.environment.config.build.cssCodeSplit) {
          if (opts.format === 'es' || opts.format === 'cjs') {
            ...
            // import order rather than chunk-iteration order.
